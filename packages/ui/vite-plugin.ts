...
function syncTypesFile(prefix: string, cfg: EmitTypesOptions): void {
  ...
  const content =
    cfg.target === 'vue' ? renderVueTypesFile(prefix, known) : renderTypesFile(prefix, known);
...
function renderVueTypesFile(prefix: string, names: readonly string[]): string {
  const sorted = [...names];
  sorted.sort();
  const imports = sorted
    .map((n) => `import type { ${ELEMENT_CLASSES[n]} } from 'luxen-ui/${n}/element';`)
...
// Generated by 'luxen-ui/vite-plugin' (emitTypes, target: 'vue'). You own this
...
import type { LuxenElement } from 'luxen-ui/luxen-element';
${imports}
